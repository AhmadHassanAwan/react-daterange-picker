  ...
  return (
    <Grid container className={classes.root}>
      <Header
        ...
        onClickPreviousYear={() => handlers.onMonthNavigate(marker, -12)}
        onClickNextYear={() => handlers.onMonthNavigate(marker, 12)}
        onClickNext={() =>
        ...
        {chunks(getDaysInMonth(date), 7).map((week, idx) => (
          <Grid key={idx} container direction="row" justify="center">
            {week.map(day => {
              ...
              const isSecondWithinRange = isStartOfRange(
                dateRange,
                new Date(day.getTime() - 24 * 60 * 60 * 1000)
              );
              const isSecondLastWithinRange = isEndOfRange(
                dateRange,
                new Date(day.getTime() + 24 * 60 * 60 * 1000)
              );
              ...
              return (
                <Day
                  ...
                  isInvalid={isInvalid}
                  secondWithinRange={
                    isSecondWithinRange && highlighted && !isRangeOneDay
                  }
                  secondLastWithinRange={
                    isSecondLastWithinRange && highlighted && !isRangeOneDay
                  }
